<template>
  <div class="ml-4">
    <div class="cursor-pointer select-none hover:bg-blue-50">
      <span v-if="hasChildren" class="inline-flex my-2">
        <svg
          @click="toggle"
          v-if="!expanded"
          data-type="toggle-expanded"
      ...
      <span class="inline-flex my-2">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 50 50"
          class="w-4 h-4 my-auto flex mx-2"
        >
          <path
        ...
        <a @click="sendToStore(item.id)" class="hover:font-semibold">{{
          item.name
        }}</a>
      </span>
    </div>
...
const sendToStore = async (parent) => {
